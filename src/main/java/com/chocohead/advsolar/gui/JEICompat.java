...
public final class JEICompat implements IModPlugin {
   public void register(IModRegistry registry) {
      ...
      this.addMachineRecipes(registry, new JEICompat.MolecularTransformerCategory(registry.getJeiHelpers().getGuiHelper()), JEICompat.MolecularTransformerRecipeWrapper.RECIPE_WRAPPER);
   }
   ...
   protected static class MolecularTransformerCategory extends JEICompat.CustomHeightDynamicCategory<IMolecularTransformerRecipeManager> {
      ...
      protected int getProcessSpeed(String name) {
         return "progress".equals(name) ? 50 : super.getProcessSpeed(name);
      }
   }
   ...
   protected static class MolecularTransformerRecipeWrapper extends IORecipeWrapper {
      ...
      MolecularTransformerRecipeWrapper(MachineRecipe<IMolecularTransformerRecipeManager.Input, ItemStack> container, IORecipeCategory<?> category) {
         ...
         this.input = Localization.translate("advanced_solar_panels.gui.input") + ' ' + inputText;
         this.output = Localization.translate("advanced_solar_panels.gui.output") + ' ' + ((ItemStack)container.getOutput()).getDisplayName();
         this.totalEU = String.format("%s %,d %s", Localization.translate("advanced_solar_panels.gui.energyPerOperation"), ((IMolecularTransformerRecipeManager.Input)container.getInput()).totalEU, Localization.translate("ic2.generic.text.EU"));
      }
