...
- **Crafting/assembly recipes** are defined as `ic2:shaped` / `ic2:shapeless` JSON under
  `data/advanced_solar_panels/recipes/` (classic hard-recipe defaults from 1.12.2). Compressor /
  metal-former extras for `iridium_ingot` and `uranium_ingot` are included too. Config toggles from
  the old `_Main` file (easy ASP recipe, disable individual recipes, etc.) are not reimplemented.
...
JEI 15.x (`mezz.jei.api`) public API. Shaped/shapeless panel and component recipes use IC2's
`ic2:shaped` / `ic2:shapeless` serializers and appear automatically in JEI's crafting category (same
as other IC2 recipes).
